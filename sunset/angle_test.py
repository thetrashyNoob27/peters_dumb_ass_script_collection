...
def main(argConfigure) -> None:
    ...
    ss.setViewPositionOnEarth(34.27, 0)

    import matplotlib.pyplot as plt

    dayOfYearIndex = [v for v in range(0, 365)]

    dayMaxAngle = np.zeros((365,))
    dayMinAngle = np.zeros((365,))
    sunshineRate = np.zeros((365,))
    logger.debug(f"dayMaxAngle shape {dayMaxAngle.shape}")
    for dayOfyear in range(0, 365):
        logger.debug(f"plotting day of year:{dayOfyear}")
        dayofyearArray = np.linspace(0, 1, 1 * 60 * 24 * 1)
        sunAngleArray = np.zeros_like(dayofyearArray)
        for idx, v in enumerate(dayofyearArray):
            sunAngleArray[idx] = ss.sunAngle(v + dayOfyear)

        dayMaxAngle[dayOfyear] = np.max(sunAngleArray)
        dayMinAngle[dayOfyear] = np.min(sunAngleArray)
        sunshineRate[dayOfyear] = (sunAngleArray > 90).sum() / dayofyearArray.shape[0]
        plt.plot(dayofyearArray, sunAngleArray)
        plt.ylim((0, 180))
        plt.title(f'sun angle at sky day:({dayOfyear})')
        plt.savefig(f"sun_angle_day_{dayOfyear}.png")
        plt.close()

    plt.plot(dayOfYearIndex, dayMinAngle, label="min")
    plt.plot(dayOfYearIndex, dayMaxAngle, label="max")
    plt.legend()
    plt.title('min/max sun angle at day')
    plt.savefig(f"sun_angle_minmax_day.png")
    plt.close()

    plt.plot(dayOfYearIndex, sunshineRate, label="<90 deg")
    plt.ylim((0, 1))
    plt.legend()
    plt.title('sunshine rate')
    plt.savefig(f"sun_shine_day_rate.png")
    plt.close()
...
# create on  [+0800]2024-05-20 03:58:47
